...
export default function DraggableObj({id, nodeProps, dragControl, refProp, otherRefs, objToRender, activeDragId, setActiveDragId, isDbLoaded}) {
  ...
  const clampToPlane = (position, sizeX, sizeZ, boxHalfSizeX, boxHalfSizeZ) => {
    position.x = Math.max(-sizeX + boxHalfSizeX, Math.min(-boxHalfSizeX, position.x));
    position.z = Math.max(-sizeZ + boxHalfSizeZ, Math.min(-boxHalfSizeZ, position.z));
  };
  ...
  useEffect(() => {
    const handleMove = (e) => {
      ...
      if (raycaster.ray.intersectPlane(plane, intersect)) {
        ...
        clampToPlane(intersect, nodeProps.sizeX, nodeProps.sizeZ, objToRender.sizeX / 2, objToRender.sizeZ/2);
